...
//function used to write the final file
...
#pragma omp master
	...
	double tstart  = CPU_TIME_W;//start to measure the computation time
...
#pragma omp parallel for firstprivate(delta_x, delta_y, x_L, y_L, I_max, n_x, n_y) collapse(2) schedule(dynamic, chunk_size)
	...
	double tend  = CPU_TIME_W; //finish to measure the computation time
	printf("process took %g of wall-clock time\n", tend - tstart);
	write_pgm_image( M, I_max, n_x, n_y, "image.pgm" ); //write the matrix on the file
	free(M);
	return 0;
}